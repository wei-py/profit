...
async function handleActivate(request, env) {
  ...
  let deviceRecord = existing.results.find(
...
async function handleAdminCreate(request, env) {
  if (!checkAdmin(request, env)) return json({ error: "未授权" }, 401);
  ...
  for (let i = 0; i < Math.min(count, 100); i++) {
    ...
    await env.DB.prepare(
      `INSERT INTO activations (${cols}) VALUES (${vals})`,
    ).run();
...
async function handleAdminDelete(request, env) {
  ...
  const act = await env.DB.prepare("SELECT id FROM activations WHERE code = ?")
    .bind(code)
    .first();
  if (!act) return json({ success: false, error: "激活码不存在" });
  ...
  const { results } = await env.DB.prepare(
    ...
    .all();
  if (results[0].cnt > 0) {
    await env.DB.prepare("UPDATE activations SET status='revoked' WHERE id = ?")
      .bind(act.id)
      .run();
    return json({
      success: true,
  ...
  await env.DB.prepare("DELETE FROM activations WHERE id = ?")
    .bind(act.id)
    .run();
