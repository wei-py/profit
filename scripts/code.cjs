#!/usr/bin/env node
/** 激活码管理工具（通过 wrangler d1 execute --remote） 用法: pnpm code create [数量] [--max-devices=N] [--expires-in=30d] pnpm code delete PFT-XXXX-XXXX pnpm code all pnpm code actived pnpm code inactive pnpm code check PFT-XXXX-XXXX */
...
function cmdCreate(args) {
  ...
  for (const arg of args) {
    if (/^\d+$/.test(arg)) {
      ...
      maxDevices = Number.parseInt(arg.split("=")[1], 10) || 1;
    }
...
function cmdDelete(args) {
  const code = args[0];
  if (!code) {
    console.error("用法: pnpm code delete <激活码>");
    process.exit(1);
  }

  const check = wrangler(
    `SELECT COUNT(*) as cnt FROM devices d JOIN activations a ON a.id=d.activation_id WHERE a.code='${code}'`,
  );
...
function cmdAll() {
  const result = wrangler(`SELECT a.code, a.status, a.max_devices, a.created_at, a.expires_at,
            COUNT(d.id) AS used
     FROM activations a
  ...
  rows.forEach((r) => {
    ...
    console.log(
      `  ${r.code}  [${r.status}]  已用${r.used}/${r.max_devices}  剩余${remaining}  ${r.expires_at || ""}`,
    );
  });
}

function cmdActived() {
  const result = wrangler(`SELECT a.code, a.status, a.max_devices,
            COUNT(d.id) AS used,
  ...
  const rows = result[0]?.results || [];
  if (rows.length === 0) {
    console.log("暂无已使用的激活码");
    return;
  }
  console.log("已使用的激活码:");
  console.log();
  rows.forEach(r =>
    console.log(`  ${r.code}  已用${r.used}/${r.max_devices}  剩余${r.remaining}`),
  );
}
...
function cmdInactive() {
  ...
  const rows = result[0]?.results || [];
  if (rows.length === 0) {
    console.log("暂无未使用的激活码");
    return;
  }
  console.log("未使用的激活码:");
  console.log();
  rows.forEach(r =>
    console.log(`  ${r.code}  可用${r.max_devices}次  ${r.expires_at || "永不过期"}`),
  );
}
...
function cmdCheck(args) {
  const code = args[0];
  if (!code) {
    console.error("用法: pnpm code check <激活码>");
    process.exit(1);
  }
  ...
  const devResult = wrangler(`SELECT d.fingerprint, d.bound_at, d.last_seen_at
  ...
  console.log(
    `额度:   ${row.used || 0} / ${row.max_devices} (剩余 ${row.remaining || row.max_devices})`,
  );
