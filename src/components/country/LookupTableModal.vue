  ...
  <dialog :open="open" class="modal">
    <div class="modal-box w-11/12 max-w-4xl max-h-[85vh] overflow-y-auto">
      ...
      <div class="flex gap-2 mb-3">
        <button class="btn btn-xs btn-ghost" @click="addRow">＋ 行</button>
        <button class="btn btn-xs btn-ghost" @click="addCol">＋ 列</button>
        <input
          v-model="newCol"
        ...
        <table class="table table-xs">
          ...
          <tbody>
            <tr v-for="(row, i) in rowsLocal" :key="i">
              ...
              <td>
                <button class="btn btn-ghost btn-xs text-error" @click="delRow(i)">🗑️</button>
              </td>
            </tr>
          </tbody>
        </table>
