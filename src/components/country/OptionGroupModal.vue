    ...
    <div class="modal-box max-w-2xl max-h-[85vh] overflow-y-auto">
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-semibold" data-tour="opt-items"
            >选项值（{{ items.length }}）</span
          >
          <button class="btn btn-xs btn-primary" @click="addItem">＋</button>
        </div>
        <table v-if="items.length" class="table table-xs">
          ...
          <tbody ref="itemsTableBodyRef">
            <tr v-for="(item, i) in items" :key="i" class="sortable-item">
              ...
              <td>
                <button class="btn btn-ghost btn-xs text-error" @click="delItem(i)">🗑️</button>
              </td>
            </tr>
          </tbody>
        </table>
