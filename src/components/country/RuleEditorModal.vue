  ...
  <dialog :open="open" class="modal">
    <div class="modal-box w-11/12 max-w-3xl max-h-[85vh] overflow-y-auto">
      ...
      <div class="grid grid-cols-4 gap-2 mb-4">
        ...
        <div>
          <label class="label py-0 text-xs">费用名称</label
          ><input v-model="form.费用名称" class="input input-bordered input-sm w-full" />
        </div>
        <div>
          <label class="label py-0 text-xs">顺序</label
          ><input
            v-model="form.计算顺序"
            type="number"
      ...
      <fieldset class="fieldset mb-3">
        ...
        <template v-if="form.计算方式 === '查表'">
          <div class="grid grid-cols-2 gap-2">
            <div>
              ...
              ><select v-model="form.查表名称" class="select select-bordered select-sm w-full">
                <option value="">—</option>
            ...
            <div>
              ...
              ><select v-model="form.匹配方式" class="select select-bordered select-sm w-full">
                <option value="">—</option>
                <option>精确</option>
                <option>区间</option>
              </select>
            ...
            <div>
              <label class="label py-0 text-xs">输入映射</label
              ><input v-model="form.输入映射" class="input input-bordered input-sm w-full" />
            </div>
            <div>
              <label class="label py-0 text-xs">输出列</label
              ><input v-model="form.输出列" class="input input-bordered input-sm w-full" />
            </div>
          </div>
        </template>
        <template v-if="form.计算方式 === '百分比'">
          <div class="grid grid-cols-3 gap-2">
            ...
            <div>
              <label class="label py-0 text-xs">固定%值</label
              ><input v-model="form.百分比值" class="input input-bordered input-sm w-full" />
            </div>
            <div>
        ...
        <template v-if="form.计算方式 === '固定值'">
          <div>
            <label class="label py-0 text-xs">固定金额</label
            ><input v-model="form.固定金额" class="input input-bordered input-sm w-32" />
          </div>
        </template>
        ...
        <template v-if="form.计算方式 === '公式'">
          <div>
            ...
            ><input v-model="form.公式" class="input input-bordered input-sm w-full font-mono" />
          </div>
        </template>
      </fieldset>
