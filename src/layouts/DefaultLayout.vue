...
function goTab(path) { router.push(path) }
</script>

<template>
  <div class="flex flex-col h-screen bg-base-200">
    <!-- 顶部栏 -->
    <header class="flex items-center justify-between h-12 px-4 bg-base-100 border-b border-base-300 flex-shrink-0">
      <div class="flex items-center gap-1">
        <span class="font-bold text-lg mr-4">利润工具</span>
        <button
          v-for="t in tabs" :key="t.path"
          class="btn btn-sm" :class="activeTab === t.path ? 'btn-primary' : 'btn-ghost'"
          @click="goTab(t.path)"
        >{{ t.label }}</button>
      </div>
      <div class="flex items-center gap-1">
        <button class="btn btn-ghost btn-sm btn-circle" @click="toggleTheme()" :title="isDark ? '浅色' : '深色'">
        ...
        <div class="dropdown dropdown-end">
          ...
          <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-50 w-52 p-2 shadow border mt-1">
